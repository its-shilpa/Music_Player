...
// Artist photo URLs (Wikipedia/public domain images)
const ARTIST_PHOTOS = {
...
function ArtistBubble({ artist, onClick }) {
  const [photoSrc, setPhotoSrc] = useState(ARTIST_PHOTOS[artist.name] || null);
  const fallback = makeArtistFallbackSVG(artist.name, artist.color);
      ...
      <div className="artist-bubble-avatar">
        {photoSrc ? (
          ...
          <img
            src={fallback}
            alt={artist.name}
            className="artist-photo artist-photo-fallback"
          />
        )}
      </div>
...
function SongRow({ song, isActive, onPlay }) {
  const [imgSrc, setImgSrc] = useState(`/images/${song.name}.jpg`);
  ...
  return (
    <div className={`song-row ${isActive ? "active" : ""}`} onClick={() => onPlay(song.id)}>
      <div className="song-row-art">
        <img
          src={imgSrc}
          onError={() => setImgSrc(makeFallbackSVG(song.name, song.color))}
          alt={song.name}
        />
      ...
      <div className="song-row-info">
        ...
        <div className="song-row-artist">{artistLabel}</div>
      </div>
...
export default function App() {
  ...
  // Reset home page when filters change
  useEffect(() => { setHomePage(1); }, [searchQuery, activeGenre, selectedArtist]);

  // Songs that share at least one artist with the current song (excluding current)
          ...
          {view === "home" && (
            <div className="home-view">
              ...
              <nav className="home-nav">
                ...
                <button className="theme-toggle-home" onClick={() => setDarkMode((d) => !d)} title="Toggle theme">
                  {darkMode ? "☀️" : "🌙"}
                </button>
              </nav>
              ...
              <div className={`home-scroll-area${isPlaying ? " has-mini-player" : ""}`}>
                ...
                {/* Artists horizontal row with photos */}
              ...
              {/* ── Sticky footer mini player (JioSaavn style) ── */}
              {isPlaying && (
                <div className="mini-player-bar" onClick={() => setView("player")}>
                  <MiniPlayerThumb song={currentSong} />
          ...
          {view === "player" && (
            <div className="player-view">
              ...
              <div className="player-scroll-area">
                ...
                {/* Related songs only — with pagination */}
                {relatedSongs.length > 0 && (
                  <div className="related-section">
                    <h2 className="related-title">
                      More by {currentSong.artists.length === 1
