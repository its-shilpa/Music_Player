...
function ArtistBubble({ artist, onClick }) {
  const [photoSrc, setPhotoSrc] = useState(
    ARTIST_PHOTOS[artist.name] ||
      makeArtistFallbackSVG(artist.name, artist.color),
  );
  return (
    <button
      type="button"
    ...
    >
      <div className="artist-bubble-avatar">
        <img
          ...
          onError={() =>
            setPhotoSrc(makeArtistFallbackSVG(artist.name, artist.color))
          }
        />
      </div>
...
export default function App() {
  ...
  useEffect(() => {
    async function loadSongs() {
      ...
      const normalizedSongs = result.map((song, index) => ({
        id: song.id ?? index,
        name: song.name ?? song.title,
        artists: Array.isArray(song.artists) ? song.artists : [song.artist],
        color: song.color ?? "#00d4ff",
        genre: song.genre ?? "Bollywood",
        image: song.image ?? null,
        preview: song.preview ?? song.url ?? null,
      }));
  ...
  const relatedSongs = currentSong
    ? songs.filter(
        (s) =>
          s.id !== currentSong.id &&
          s.artists.some((a) => currentSong.artists.includes(a)),
      )
    : [];
  ...
  const playSongFromRelated = (id) => {
    ...
    const newRelated = songs.filter(
      (s) =>
        s.id !== id &&
        s.artists.some((a) =>
          songs.find((x) => x.id === id)?.artists.includes(a),
        ),
    );
        ...
        <div className="overlay">
          ...
          {view === "home" && (
            <div className="home-view">
              ...
              <div
                className={`home-scroll-area${isPlaying ? " has-mini-player" : ""}`}
              >
          ...
          {view === "player" && currentSong && (
            <div className="player-view">
              <div className="player-nav">
                <button className="player-back-btn" onClick={goHome}>
                  ← Home
                </button>
              ...
              <div className="player-scroll-area">
                <div
                  className={`player-card-wrap ${isPlaying ? "playing" : ""}`}
                >
                  <div className="player-card">
                    <div className="album-container">
                      <div className="album-art">
                        ...
                        <img
                          src={imgSrc || makeFallbackSVG(currentSong.name, currentSong.color)}
                          onError={() =>
                            setImgSrc(
                              makeFallbackSVG(
                                currentSong.name,
